  ...
  div
    ...
    Card.flow(v-if="step.current===0")
      ...
      Form
          FormItem(label="選擇作家" :label-width="80")
            Select(v-model="createData.author" filterable placeholder="請選擇作家" not-found-text="找不到內容" size="large")
      ...
      div
        Form
          FormItem(label="故事名稱" :label-width="80")
            Input(placeholder="請輸入故事名稱" v-model="createData.title")
    ...
    Card.flow(v-if="step.current===3")
      h3(slot="title" style={display:"flex" , justifyContent:"left" , alignItems:"center"})
        ...
        span  確認資料
      Card
        h3 {{createData.author}}_{{createData.title}}
    ...
    export default {
        ...
        methods: {
            uploadStoryData(){
                ...
                axios.post('/api/uploadStoryData', vm.createData)
        ...
        mounted() {
            let vm = this;
            vm.getAllAuthor()
            console.log(vm.storyData.author)
            vm.loadMap();
